...
from sklearn.metrics import confusion_matrix, f1_score, roc_auc_score, precision_score, recall_score, roc_curve, average_precision_score, precision_recall_curve
import pandas as pd
...
def calculate_classification_metrics(y_test, y_pred, y_probs=None):
    ...
    metrics = {
        ...
        'PR AUC':  average_precision_score(y_test, y_probs) if y_probs is not None else None,
    ...
    if y_probs is not None:
        ...
        prec, rec, pr_thr = precision_recall_curve(y_test, y_probs)
        metrics['ROC Curve'] = {
            'fpr': fpr,
            'tpr': tpr,
            'thresholds': thresholds
        }
        metrics['PR Curve']  = {'precision': prec, 'recall': rec, 'thresholds': pr_thr}
...
def train_evaluate_models(models: list, X_train, y_train, X_test, y_test, seed=None):
    ...
    numeric_only = {name: _flatten_numeric(m) for name, m in all_metrics.items()}
    metrics_df = pd.DataFrame.from_dict(numeric_only, orient='index')

    # упорядочим столбцы, если они есть
    cols_order = ['ROC AUC', 'PR AUC', 'F1 Score', 'Precision', 'Recall', 'Accuracy']
